# app/settings.py (РЕВОЛЮЦИОННАЯ ВЕРСИЯ v4.2 - Hyperliquid Integration)
"""
INTELLIGENT CRYPTO MONITOR - Configuration

НОВЫЕ ВОЗМОЖНОСТИ v4.2:
✅ Hyperliquid Exchange Monitoring - мониторинг Hyperliquid DEX
✅ Whale Activity Detection - детектирование активности китов
✅ Liquidation Detection - определение ликвидаций через паттерны
✅ Funding Rates Analysis - анализ funding rates
✅ Volume Spike Detection - детектирование аномальных объемов
...
MIN_USD = float(os.getenv('MIN_USD', '50000'))
MIN_USD_FLOOR = float(os.getenv('MIN_USD_FLOOR', '10000'))
...
START_FROM_MINUTES_AGO = int(os.getenv('START_FROM_MINUTES_AGO', '15'))
...
ENABLE_IMAGES = int(os.getenv('ENABLE_IMAGES', '0')) == 1
...
DEBUG_FILTERS = int(os.getenv('DEBUG_FILTERS', '0')) == 1
...
RETRY_TIMEOUT = int(os.getenv('RETRY_TIMEOUT', '15'))

SOLANA_RATE_LIMIT_REQUESTS = int(os.getenv('SOLANA_RATE_LIMIT_REQUESTS', '50'))
SOLANA_RATE_LIMIT_WINDOW = int(os.getenv('SOLANA_RATE_LIMIT_WINDOW', '60'))
SOLANA_RETRY_ON_429 = int(os.getenv('SOLANA_RETRY_ON_429', '1')) == 1
...
TRADING_MONITORED_ASSETS = [a.strip() for a in os.getenv('TRADING_MONITORED_ASSETS', 'BTC,ETH,SOL,BNB,XRP,ADA,AVAX,DOT,MATIC,LINK,UNI,AAVE,ARB,OP').split(',') if a.strip()]
...
SMART_DISCOVERY_MIN_PRICE_CHANGE = float(os.getenv('SMART_DISCOVERY_MIN_PRICE_CHANGE', '3.0'))
SMART_DISCOVERY_MAX_TOKENS_TO_ANALYZE = int(os.getenv('SMART_DISCOVERY_MAX_TOKENS_TO_ANALYZE', '20'))

SMART_DISCOVERY_MIN_WALLET_ROI = float(os.getenv('SMART_DISCOVERY_MIN_WALLET_ROI', '1.0'))
SMART_DISCOVERY_MIN_WIN_RATE = float(os.getenv('SMART_DISCOVERY_MIN_WIN_RATE', '0.60'))
SMART_DISCOVERY_MIN_TRADES = int(os.getenv('SMART_DISCOVERY_MIN_TRADES', '5'))
SMART_DISCOVERY_LOOKBACK_DAYS = int(os.getenv('SMART_DISCOVERY_LOOKBACK_DAYS', '90'))

SMART_DISCOVERY_MAX_WALLETS_PER_RUN = int(os.getenv('SMART_DISCOVERY_MAX_WALLETS_PER_RUN', '50'))
SMART_DISCOVERY_MAX_NEW_WALLETS = int(os.getenv('SMART_DISCOVERY_MAX_NEW_WALLETS', '10'))
...
VALIDATION_MAX_INACTIVE_DAYS = int(os.getenv('VALIDATION_MAX_INACTIVE_DAYS', '30'))
VALIDATION_MIN_SCORE_TO_KEEP = int(os.getenv('VALIDATION_MIN_SCORE_TO_KEEP', '30'))
VALIDATION_MIN_TRADES_TO_KEEP = int(os.getenv('VALIDATION_MIN_TRADES_TO_KEEP', '3'))
VALIDATION_MIN_WIN_RATE_TO_KEEP = float(os.getenv('VALIDATION_MIN_WIN_RATE_TO_KEEP', '0.40'))
VALIDATION_MIN_ROI_TO_KEEP = float(os.getenv('VALIDATION_MIN_ROI_TO_KEEP', '-0.20'))
...
VALIDATION_NOTIFY_THRESHOLD = int(os.getenv('VALIDATION_NOTIFY_THRESHOLD', '5'))
...
PERFORMANCE_CHECK_INTERVALS = [int(x) for x in os.getenv('PERFORMANCE_CHECK_INTERVALS', '1,4,24,72,168').split(',')]

PERFORMANCE_HISTORY_SIZE = int(os.getenv('PERFORMANCE_HISTORY_SIZE', '1000'))
PERFORMANCE_LOOKBACK_DAYS = int(os.getenv('PERFORMANCE_LOOKBACK_DAYS', '90'))

PERFORMANCE_SUCCESS_THRESHOLD = float(os.getenv('PERFORMANCE_SUCCESS_THRESHOLD', '0.05'))
PERFORMANCE_MIN_CONFIDENCE_FOR_TRACKING = int(os.getenv('PERFORMANCE_MIN_CONFIDENCE_FOR_TRACKING', '50'))
...
ADAPTIVE_BASE_MIN_CONFIDENCE = int(os.getenv('ADAPTIVE_BASE_MIN_CONFIDENCE', '70'))
ADAPTIVE_BASE_MIN_SIZE_REL = float(os.getenv('ADAPTIVE_BASE_MIN_SIZE_REL', '0.01'))
ADAPTIVE_BASE_MIN_VOLUME_24H = float(os.getenv('ADAPTIVE_BASE_MIN_VOLUME_24H', '1000000'))

ADAPTIVE_BULL_THRESHOLD = float(os.getenv('ADAPTIVE_BULL_THRESHOLD', '10.0'))
ADAPTIVE_BEAR_THRESHOLD = float(os.getenv('ADAPTIVE_BEAR_THRESHOLD', '-10.0'))
ADAPTIVE_MARKET_REGIME_UPDATE_HOURS = int(os.getenv('ADAPTIVE_MARKET_REGIME_UPDATE_HOURS', '6'))

ADAPTIVE_BULL_CONFIDENCE_MODIFIER = int(os.getenv('ADAPTIVE_BULL_CONFIDENCE_MODIFIER', '-5'))
ADAPTIVE_BULL_SIZE_REL_MODIFIER = float(os.getenv('ADAPTIVE_BULL_SIZE_REL_MODIFIER', '-0.02'))
ADAPTIVE_BULL_VOLUME_MODIFIER = float(os.getenv('ADAPTIVE_BULL_VOLUME_MODIFIER', '0.8'))

ADAPTIVE_BEAR_CONFIDENCE_MODIFIER = int(os.getenv('ADAPTIVE_BEAR_CONFIDENCE_MODIFIER', '+10'))
ADAPTIVE_BEAR_SIZE_REL_MODIFIER = float(os.getenv('ADAPTIVE_BEAR_SIZE_REL_MODIFIER', '+0.05'))
ADAPTIVE_BEAR_VOLUME_MODIFIER = float(os.getenv('ADAPTIVE_BEAR_VOLUME_MODIFIER', '1.5'))

ADAPTIVE_MIN_SIGNALS_FOR_ADAPTATION = int(os.getenv('ADAPTIVE_MIN_SIGNALS_FOR_ADAPTATION', '20'))
ADAPTIVE_LOW_ACCURACY_THRESHOLD = float(os.getenv('ADAPTIVE_LOW_ACCURACY_THRESHOLD', '0.40'))
ADAPTIVE_HIGH_ACCURACY_THRESHOLD = float(os.getenv('ADAPTIVE_HIGH_ACCURACY_THRESHOLD', '0.65'))
ADAPTIVE_ACCURACY_ADJUSTMENT = int(os.getenv('ADAPTIVE_ACCURACY_ADJUSTMENT', '5'))
...
LEARNING_MIN_SAMPLES = int(os.getenv('LEARNING_MIN_SAMPLES', '30'))
...
WALLET_DB_TYPE = os.getenv('WALLET_DB_TYPE', 'json')
...
WALLET_BASE_SCORE = int(os.getenv('WALLET_BASE_SCORE', '50'))
WALLET_INITIAL_SCORE = WALLET_BASE_SCORE
...
WALLET_SCORE_DECAY_PER_DAY = float(os.getenv('WALLET_SCORE_DECAY_PER_DAY', '1.0'))

WALLET_MAX_TRACKED = int(os.getenv('WALLET_MAX_TRACKED', '500'))
WALLET_AUTO_PRUNE = int(os.getenv('WALLET_AUTO_PRUNE', '1')) == 1

# ============================================================================
# HYPERLIQUID EXCHANGE MONITORING (НОВОЕ v4.2)
# ============================================================================
...
HYPERLIQUID_MIN_TRADE_USD = float(os.getenv('HYPERLIQUID_MIN_TRADE_USD', '100000'))
HYPERLIQUID_TRADE_LOOKBACK_MINUTES = int(os.getenv('HYPERLIQUID_TRADE_LOOKBACK_MINUTES', '60'))
HYPERLIQUID_TRADE_CHECK_INTERVAL = int(os.getenv('HYPERLIQUID_TRADE_CHECK_INTERVAL', '180'))

HYPERLIQUID_MIN_LIQUIDATION_USD = float(os.getenv('HYPERLIQUID_MIN_LIQUIDATION_USD', '100000'))
HYPERLIQUID_LIQUIDATION_LOOKBACK_MINUTES = int(os.getenv('HYPERLIQUID_LIQUIDATION_LOOKBACK_MINUTES', '60'))
HYPERLIQUID_LIQUIDATION_CHECK_INTERVAL = int(os.getenv('HYPERLIQUID_LIQUIDATION_CHECK_INTERVAL', '300'))
HYPERLIQUID_LIQUIDATION_MIN_CONFIDENCE = float(os.getenv('HYPERLIQUID_LIQUIDATION_MIN_CONFIDENCE', '0.5'))

HYPERLIQUID_MIN_WHALE_ACTIVITY_USD = float(os.getenv('HYPERLIQUID_MIN_WHALE_ACTIVITY_USD', '500000'))
HYPERLIQUID_WHALE_ACTIVITY_LOOKBACK_MINUTES = int(os.getenv('HYPERLIQUID_WHALE_ACTIVITY_LOOKBACK_MINUTES', '60'))
HYPERLIQUID_WHALE_ACTIVITY_CHECK_INTERVAL = int(os.getenv('HYPERLIQUID_WHALE_ACTIVITY_CHECK_INTERVAL', '300'))
HYPERLIQUID_WHALE_MIN_CONFIDENCE = float(os.getenv('HYPERLIQUID_WHALE_MIN_CONFIDENCE', '0.5'))

HYPERLIQUID_FUNDING_CHECK_INTERVAL = int(os.getenv('HYPERLIQUID_FUNDING_CHECK_INTERVAL', '3600'))
HYPERLIQUID_EXTREME_FUNDING_THRESHOLD = float(os.getenv('HYPERLIQUID_EXTREME_FUNDING_THRESHOLD', '0.0001'))
HYPERLIQUID_MIN_OI_USD = float(os.getenv('HYPERLIQUID_MIN_OI_USD', '10000000'))

HYPERLIQUID_VOLUME_SPIKE_MULTIPLIER = float(os.getenv('HYPERLIQUID_VOLUME_SPIKE_MULTIPLIER', '3.0'))
HYPERLIQUID_VOLUME_SPIKE_CHECK_INTERVAL = int(os.getenv('HYPERLIQUID_VOLUME_SPIKE_CHECK_INTERVAL', '600'))

HYPERLIQUID_NOTIFY_TRADES = os.getenv('HYPERLIQUID_NOTIFY_TRADES', 'true').lower() == 'true'
HYPERLIQUID_NOTIFY_LIQUIDATIONS = os.getenv('HYPERLIQUID_NOTIFY_LIQUIDATIONS', 'true').lower() == 'true'
HYPERLIQUID_NOTIFY_WHALE_ACTIVITY = os.getenv('HYPERLIQUID_NOTIFY_WHALE_ACTIVITY', 'true').lower() == 'true'
HYPERLIQUID_NOTIFY_FUNDING = os.getenv('HYPERLIQUID_NOTIFY_FUNDING', 'true').lower() == 'true'
HYPERLIQUID_NOTIFY_VOLUME_SPIKES = os.getenv('HYPERLIQUID_NOTIFY_VOLUME_SPIKES', 'true').lower() == 'true'

HYPERLIQUID_RATE_LIMIT_REQUESTS = int(os.getenv('HYPERLIQUID_RATE_LIMIT_REQUESTS', '100'))
HYPERLIQUID_RATE_LIMIT_WINDOW = int(os.getenv('HYPERLIQUID_RATE_LIMIT_WINDOW', '60'))

HYPERLIQUID_WS_ENABLED = os.getenv('HYPERLIQUID_WS_ENABLED', 'false').lower() == 'true'
HYPERLIQUID_WS_CHANNELS = [c.strip() for c in os.getenv('HYPERLIQUID_WS_CHANNELS', 'trades,liquidations').split(',')]

HYPERLIQUID_MARKETS_CACHE_TTL = int(os.getenv('HYPERLIQUID_MARKETS_CACHE_TTL', '60'))
...
def validate_config():
    ...
    if HYPERLIQUID_ENABLED:
        if HYPERLIQUID_MIN_TRADE_USD < 50000:
            warnings.append(f"HYPERLIQUID_MIN_TRADE_USD низкий ({HYPERLIQUID_MIN_TRADE_USD}) - может быть много шума")
        
        if HYPERLIQUID_EXTREME_FUNDING_THRESHOLD < 0.00005:
            warnings.append(f"HYPERLIQUID_EXTREME_FUNDING_THRESHOLD очень низкий ({HYPERLIQUID_EXTREME_FUNDING_THRESHOLD}) - много алертов")
        
        if HYPERLIQUID_RATE_LIMIT_REQUESTS > 200:
            warnings.append(f"HYPERLIQUID_RATE_LIMIT_REQUESTS высокий ({HYPERLIQUID_RATE_LIMIT_REQUESTS}) - может вызвать rate limiting")
...
def print_config():
    ...
    print("=" * 80)
    print("⚙️  КОНФИГУРАЦИЯ СИСТЕМЫ v4.2")
    print("=" * 80)
    ...
    print(f"  • MIN_USD_FLOOR: ${MIN_USD_FLOOR:,.0f}")
    ...
    if HYPERLIQUID_ENABLED:
        ...
        print(f"  • Min Trade: ${HYPERLIQUID_MIN_TRADE_USD:,.0f}")
        print(f"  • Min Liquidation: ${HYPERLIQUID_MIN_LIQUIDATION_USD:,.0f}")
        print(f"  • Min Whale Activity: ${HYPERLIQUID_MIN_WHALE_ACTIVITY_USD:,.0f}")
        print(f"  • Min OI: ${HYPERLIQUID_MIN_OI_USD:,.0f}")
        print(f"  • Extreme Funding: {HYPERLIQUID_EXTREME_FUNDING_THRESHOLD*100:.4f}%")
        print(f"  • Volume Spike: {HYPERLIQUID_VOLUME_SPIKE_MULTIPLIER}x")
        print(f"  • Интервалы проверки:")
        print(f"     - Trades: {HYPERLIQUID_TRADE_CHECK_INTERVAL}s")
        print(f"     - Liquidations: {HYPERLIQUID_LIQUIDATION_CHECK_INTERVAL}s")
        print(f"     - Whale Activity: {HYPERLIQUID_WHALE_ACTIVITY_CHECK_INTERVAL}s")
        print(f"     - Funding: {HYPERLIQUID_FUNDING_CHECK_INTERVAL}s")
        print(f"     - Volume Spikes: {HYPERLIQUID_VOLUME_SPIKE_CHECK_INTERVAL}s")
        print(f"  • Уведомления:")
        print(f"     - Trades: {'да' if HYPERLIQUID_NOTIFY_TRADES else 'нет'}")
        print(f"     - Liquidations: {'да' if HYPERLIQUID_NOTIFY_LIQUIDATIONS else 'нет'}")
        print(f"     - Whale Activity: {'да' if HYPERLIQUID_NOTIFY_WHALE_ACTIVITY else 'нет'}")
        print(f"     - Funding: {'да' if HYPERLIQUID_NOTIFY_FUNDING else 'нет'}")
        print(f"     - Volume Spikes: {'да' if HYPERLIQUID_NOTIFY_VOLUME_SPIKES else 'нет'}")
        print(f"  • WebSocket: {'✅' if HYPERLIQUID_WS_ENABLED else '❌'}")
        if HYPERLIQUID_WS_ENABLED:
            print(f"  • WS Channels: {', '.join(HYPERLIQUID_WS_CHANNELS)}")
    else:
    ...
    if TRADING_ENABLED:
        ...
        print(f"     • Отслеживаемые активы: {len(TRADING_MONITORED_ASSETS)}")
    else:
        print(f"     • Status: ❌ Отключен")
    ...
    if HYPERLIQUID_ENABLED:
        enabled_systems.append("Hyperliquid Monitor")
    
    if enabled_systems:
        print(f"✅ Активные системы: {', '.join(enabled_systems)}")
        print(f"🎯 Система работает в ИНТЕЛЛЕКТУАЛЬНОМ режиме v4.2")
    else:
...
def get_all_settings() -> Dict:
    ...
    return {
        ...
        "trading_system": {
            ...
            "monitored_assets": len(TRADING_MONITORED_ASSETS),
        },
        "smart_discovery": {
        ...
        "hyperliquid": {
            "enabled": HYPERLIQUID_ENABLED,
            "api_url": HYPERLIQUID_API_URL,
            "min_trade_usd": HYPERLIQUID_MIN_TRADE_USD,
            "min_liquidation_usd": HYPERLIQUID_MIN_LIQUIDATION_USD,
            "min_whale_activity_usd": HYPERLIQUID_MIN_WHALE_ACTIVITY_USD,
            "min_oi_usd": HYPERLIQUID_MIN_OI_USD,
            "extreme_funding_threshold": HYPERLIQUID_EXTREME_FUNDING_THRESHOLD,
            "volume_spike_multiplier": HYPERLIQUID_VOLUME_SPIKE_MULTIPLIER,
            "websocket_enabled": HYPERLIQUID_WS_ENABLED,
        }
    }
